...
// furrowProvider is the real store client: every read and write shells out to
...
// newFurrowProvider probes the store and performs the initial load, so a
// missing binary or an unreadable board fails at startup rather than at the
// first keypress.
func newFurrowProvider(perf func(op string, d time.Duration)) (*furrowProvider, error) {
	c := newFurrowClient()
	c.perf = perf
	p := &furrowProvider{c: c}
...
func (p *furrowProvider) Board() *Board {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.b
}

func (p *furrowProvider) Live() bool { return true }

func (p *furrowProvider) Reload() error {
	b, err := p.load()
	if err != nil {
		return err
	}
...
func (p *furrowProvider) Sync() error {
...
// bodies included.
func (p *furrowProvider) load() (*Board, error) {
	var (
		cfg   boardJSON
		rows  []taskJSON
		boxes []epicJSON
	...
	lanes := make([]Lane, 0, len(cfg.Lanes))
	...
	for _, r := range rows {
		...
		for _, c := range r.Checklist {
			t.Checklist = append(t.Checklist, ChecklistItem{Text: c.Text, Done: c.Done})
		}
	...
	epics := make([]EpicInfo, 0, len(boxes))
	for _, e := range boxes {
	...
	return newStoreBoard(lanes, tasks, epics, cfg.Writable, cfg.SchemaState), nil
}
...
func readBodies(store string, rows []taskJSON, tasks []*Task) error {
	type job struct {
		path string
		dst  *Task
	}
...
func (p *furrowProvider) PersistMove(id, lane, beforeID, afterID string) ([]string, error) {
...
func (p *furrowProvider) PersistDone(id string) error {
	_, err := p.c.run("done", "done", id)
	return err
}

func (p *furrowProvider) PersistCheck(id string, i int, done bool) error {
...
func (p *furrowProvider) PersistBody(id, body string) error {
