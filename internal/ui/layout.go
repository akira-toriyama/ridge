...
func (m *measurer) rebind(g *board.Graph, th *theme) {
	m.g, m.th = g, th
	clear(m.cache)
}

func (m *measurer) height(t *board.Task, w int) int {
...
func layCards(tasks []*board.Task, scroll, x, top, bot, colW int, ms *measurer) []cardBox {
	var out []cardBox
	y := top
...
func maxScrollFor(tasks []*board.Task, top, bot, colW int, ms *measurer) int {
	if len(tasks) == 0 {
		return 0
	}
...
func scrollToShow(tasks []*board.Task, idx, scroll, top, bot, colW int, ms *measurer) int {
