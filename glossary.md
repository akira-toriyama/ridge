...
| **front-end** | furrow を **CLI/JSON 経由で**駆動するもの（import 禁止の契約と理由は [CLAUDE.md](CLAUDE.md)）。 |
| **Provider** | ridge がタスクを読み書きする唯一の口（interface）。port は `internal/board` が宣言し、adapter は `internal/store/furrowstore`（`furrow` を exec する実装）と `internal/store/memstore`（fixture）の 2 つ。Model と Persist* の分業（適用は Model・記録は store・Persist は UI thread の外）は `internal/board/provider.go` 冒頭が正本。例外は **store-first** の族（下）。 |
...
| **orientation**（Graph の向き） | Graph の階層をどの画面軸に並べるか。**top-down**（既定・上が blocker / 下が「閉じると動き出すもの」）と **left-right**（左が blocker / 右）の2値で、`o` で切替（`-graphlr` で headless）。**位置と矢頭の両方**が常に同じ方向を指す（矢頭は `▼` / `▶`）ので、向きを変えても矢印の意味を覚え直さなくていい。与えられた軸を使い切り**もう一方を交渉する**関係も入れ替わる: top-down は幅が与えられてタイトル行数を交渉し、left-right は高さが与えられて箱の**幅**を交渉する。幅に入り切らない段は落として件数をヘッダに出し、**描く窓はカーソルに追随する**（選択が窓の外へ出ない）。設計時（2026-08）の実データの ego graph は最長 6 段（最長鎖 5 辺）で、6 段 × 下限幅がちょうど 240 桁に収まる — それを超える盤面が「段を落とす」経路に入る（`graphview.go`）。セッションを跨いで保存しない（hop radius と同じ）。 |
| **Map**（依存マップ） | **全依存クラスタを一画面で俯瞰するビュー**。Graph が「1タスク起点」なのに対し、こちらは起点を持たない。`T`（`t` = そのタスクの依存ツリー、の全体版）。線は引かず、**インデント = 深さ・`←` = blocker の名指し**で表す。各クラスタと全体に出す数字（unblocked / blocked / 最多 blocker / 最長鎖）は**同じ画面の行の印から数え直せる**（`unblocked + blocked + done = ノード数` — 純トポロジーで数えると `v` が並ぶ行の上に「7 unblocked」が出る）。2 経路で到達するノードも 1 回しか出ない。filter は行を消さず mute して header で数える。 |
...
| **保存ビュー**（saved view） | **{layout, q, sort, slice} の束に名前を付けたもの**（GH Projects の view タブ相当）。正本は `~/.config/ridge/views.toml` の `[[view]]` — **ridge が書く唯一のファイル**で、書くのは明示の `V` だけ（config でなく保存データ、の整理）。furrow の board には置かない — 見せ方は front-end の所有物で、vista と共有したくなったら furrow 側に要望を出し直す。タイトル行の Board\|Table の右がタブ帯: `1`-`9` 切替・`V` 保存（roadmap ビュー内でも効く）。**上限 9 個** — digit がタブへ届く唯一の経路なので、10 個目の新規は理由つきで拒否し、手書きの 10 個目以降は表示のみ（`viewtabs.go`）。読み込みは起動時 1 回・書くのは `V` だけで、file の規則（clamp・全量書き戻し・last-writer-wins・symlink 貫通）は `internal/views/views.go`（package 冒頭と Load / Save の doc comment）が正本。layout は board\|table\|roadmap の 3 値（graph は起点 task が要り、map/boxes は population 切替なので対象外）。 |
...
| **review stamp（`i`）** | 選択タスクの `reviewed` を今に打つ（`furrow review <id>` 相当）。**`updated` は動かない** — review は内容を変えない、が furrow の定義（実 binary で実測・contract test が pin）。楽観適用 + persist キューで、盤面上の変化は note と peek の刻印行の `reviewed <ago>` だけ（未 stamp のタスクには行を出さない — 「reviewed never」は review しない盤面では全カードへの小言になる）。repo / epic の review（`furrow review <repo\|epic>`）は対象外。 |
| **revisit lens（`f`）** | **`furrow revisit -q` を verdict にするトグル**。立てると盤面は revisit が flag した open task だけになり、filter 行に `↻ revisit` チップ、peek に `↻ revisit <code> <detail> · …` 行（code/detail は furrow のまま — `furrow vocab revisit-codes`）。typed query と slice は `-q` として furrow に渡り **AND は furrow 側**で合成する（ridge は verdict を交差させない）。filter 語ではなく lens なのは、dep_done 等が `-q` で表現できない別の read だから。保存ビューの束には**含めない**（盤面への問いであって盤面の見え方ではない）。`-revisit` で headless（`-filter` と同じ Init 経由）。 |
...
| `↻` | **revisit lens** が立っている印。filter 行のチップと peek の理由行に出る。カードには出さない — 生き残ったこと自体が印で、理由は peek の仕事。 |
| `[0/7]` | チェックリストの進捗。 |
...
| 用語 | 意味 |
|---|---|
| **measurer** | カード高さのキャッシュ（`internal/ui/layout.go` — レイアウトと当たり判定は同じ計測結果から作る）。**フレームを跨いで持つ** — フレーム単位にするとスクロール上限の計算が毎フレーム全カードを描画し、658 タスクで 36ms/frame（実測。跨いで 3.3ms）。`recompute()` で破棄する — 中身が変わったのに古い高さを使うと描画と当たり判定がズレる。 |
