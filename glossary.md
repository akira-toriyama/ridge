...
| `▶` / `◆` | slice パネルの epic 行の lifecycle 印。`▶` = その repo が今それで作業している box（`furrow brief` と同じ字）、`◆` = pinned。どちらも `epic ls --all --json` の `active`/`pinned` をそのまま出す。 |
